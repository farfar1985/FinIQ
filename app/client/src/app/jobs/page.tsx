...
function JobExportBar({ job }: { job: Job }) {
  if (!job.result) return null;

  const hasData = job.result.data && Array.isArray(job.result.data) && job.result.data.length > 0;
  const ts = new Date().toISOString().slice(0, 10);
  const baseName = `finiq_job_${job.id.slice(0, 8)}_${ts}`;

  function exportText() {
    const content = job.result!.summary || "No summary available.";
  ...
  function exportXLSX() {
    if (!hasData) return;
    const data = job.result!.data as Record<string, unknown>[];
    // Build worksheet with Mars branding
    const rows: unknown[][] = [
      ["FinIQ — Mars, Incorporated"],
      ["Confidential"],
      [],
      [`Job: ${job.query}`],
      [`Agent: ${job.agent_name} | Completed: ${new Date(job.updated_at).toLocaleString()}`],
      [],
    ];
    ...
    for (const row of data) {
      rows.push(headers.map((h) => {
        const val = row[h];
        if (typeof val === "object" && val !== null) return (val as Record<string, unknown>).tagline || JSON.stringify(val);
        return val;
      }));
    }
    rows.push([]);
    rows.push([`Generated by FinIQ | ${new Date().toISOString()}`]);

    const ws = XLSX.utils.aoa_to_sheet(rows);
    ws["!cols"] = headers.map((h) => ({ wch: Math.min(Math.max(h.length, 12) + 2, 40) }));
  ...
  return (
    <div className="flex items-center gap-2 border-t border-border pt-3">
      ...
      <button
        onClick={exportText}
        className="rounded border border-border px-2.5 py-1 text-[10px] text-muted-foreground transition-colors hover:bg-accent hover:text-foreground"
      >
        Report (.txt)
      </button>
      <button
