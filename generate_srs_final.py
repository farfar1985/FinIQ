...
FINAL UNIFIED VERSION — merges SRS v2.1 base + Addendum A (Databricks) + dual-data-mode
Version 3.0
"""
...
def add_title_page():
    ...
    # Metadata table
    meta = [
        ("Document Version", "3.0 (Final Unified)"),
...
add_para(
    "This is the final unified specification incorporating all Databricks/FinSight data layer "
    "requirements and dual-mode operation (simulated and real data) into a single document.",
    italic=True
)
...
add_bullet("Internal financial data ingestion via Databricks/FinSight (primary) and Excel workbooks (fallback)")
...
add_bullet("Budget variance reporting (actual vs. replan from Databricks)")
...
add_bullet("Dual-mode operation: simulated data for development/testing, real Databricks data for production")

doc.add_heading("1.2.2 Out of Scope", level=3)
add_bullet("Direct SAP/ERP integration (financial data provided via Databricks/FinSight data layer; Excel exports retained as fallback during transition period)")
...
add_para(
    ...
    "system attributes. Sections 4-5 cover data model and system architecture. Section 6 covers the "
    "Databricks/FinSight schema and precomputed view mapping. Section 7 covers dual-mode operation "
    "(simulated vs. real data). Sections 8-10 cover deployment, phased rollout, and acceptance "
...
doc.add_heading("2.1.3 Current System: FinSight Data Layer", level=3)
...
doc.add_heading("2.1.4 Gap Analysis", level=3)
add_table(
...
add_bullet("Dual-Mode Operation: System operates against both simulated data (development/testing) and real Databricks data (production) via a single configuration toggle")
...
add_bullet("LLM access via Azure OpenAI Foundry with data processing agreement ensuring no training on Mars data")
add_bullet("Authentication must integrate with Mars's existing Azure Entra ID (SSO)")
add_bullet("Primary data ingestion via direct Databricks/FinSight connectivity (finsight_core_model_mvp3 schema). Excel ingestion retained as fallback path during transition and for any data not yet available in Databricks. No direct SAP/ERP integration (SAP data flows through Databricks).")
...
add_bullet("Mars will provide and maintain Databricks/FinSight connectivity credentials (service principal or managed identity) with read access to the finsight_core_model_mvp3 schema. The schema structure (20 tables/views) will remain stable or changes will be communicated in advance. Excel workbook ingestion is retained as a fallback and will follow the existing naming convention.")
add_bullet("The external dimension tables referenced by FinSight views (Dimensions_View_Date_Map, Dimensions_Date, Dimensions_Entity, Dimensions_Account) will be accessible to FinIQ with the same credentials as the finiq_ prefixed tables.")
...
add_bullet("Databricks connection administration panel for managing data source connectivity")
...
add_para("Databricks / FinSight Data Layer", bold=True)
add_table(
    ["Interface", "Protocol", "Description", "Authentication"],
    [
        ["Databricks SQL Connector", "ODBC/JDBC", "Read access to finiq_ tables and views", "Service principal / managed identity"],
        ["Unity Catalog REST API", "HTTPS", "Schema discovery and metadata retrieval", "Service principal / OAuth 2.0"],
    ],
...
add_req_table("FR1.1", "Internal Financial Data Ingestion (Databricks Primary)",
...
add_para(
    ...
    "When Databricks connectivity is available, the engine queries precomputed FinSight views "
    "directly, eliminating the Excel preprocessing step."
)

add_req_table("FR2.1", "Period End Summary Generation (Databricks-Enabled)",
    "The system shall generate executive performance summaries for any organizational unit, time period, "
    "and output format. When Databricks connectivity is available, the system shall query the precomputed "
    "FinSight views directly: finiq_vw_pl_entity for P&L by entity (maps to P&L sheet), "
    "finiq_vw_pl_brand_product for P&L by brand/product (maps to Product and Brand sheets), and "
    "finiq_vw_ncfo_entity for NCFO (maps to NCFO sheet). Each view provides Date_ID, Entity_Alias, "
    "Account_Alias, and four value columns (YTD_LY_Value, YTD_CY_Value, Periodic_LY_Value, "
    "Periodic_CY_Value) that directly feed the KPI narrative generation pipeline. This eliminates the "
    "Excel preprocessing step from the current 10-step pipeline. Six KPI narratives (Organic Growth, "
    "MAC Shape, A&CP Shape, CE Shape, Controllable Overhead Shape, NCFO) shall be generated in parallel, "
    "with trend analysis taglines injected from multi-period historical data. Output formats: Summary, "
    "What's Working Well, What's Not Working Well. When Databricks is unavailable, the system shall fall "
    "back to the Excel-based pipeline.",
    priority="Critical")
...
add_req_table("FR2.2", "Configurable KPI Framework",
    ...
    "The finiq_account_formula table shall serve as the authoritative source for KPI calculation logic, "
    "enabling dynamic KPI configuration from Databricks metadata.")
...
add_req_table("FR6.1", "Amira Financial Forecasting Integration (Three-Way Comparison)",
    "The system shall call the Amira Financial Forecasting API to enrich reports with forward-looking "
    "projections. In addition, the system shall leverage the finiq_financial_replan table for Mars's "
    "internal replan/budget data. This enables a three-way comparison: Actual (from finiq_financial_cons) "
    "vs. Replan (from finiq_financial_replan) vs. Amira Forecast (from Forecasting API). Example output: "
    "'Q2 Organic Growth was 6.3% (Actual) vs. 5.5% (Mars Replan) vs. 6.1% (Amira Forecast). Actual "
    "exceeded both targets. Based on current trends, Amira projects 5.8% for Q3.' Confidence intervals "
    "shall be displayed for the Amira Forecast; replan figures shall be labeled as Mars internal budget "
    "targets. Forecasts shall be clearly labeled as projections.",
    priority="Critical")
...
add_para(
    "The user interface shall be dynamic, adaptive, and configurable \u2014 enabling users to personalize their "
    "experience, interact with data in real time, and work within a context-aware workspace that adjusts "
    "its layout and components based on user intent and workflow stage. The UI shall support progressive "
    "disclosure, multi-panel workspaces, and persistent state management."
)
...
add_req_table("FR8.7", "Context-Aware UI Rendering",
    "The system shall dynamically adjust the UI layout based on the detected user intent. When the "
    "query interface classifies a request as retrieval, the UI shall foreground the document viewer; "
    "for analysis requests, the charting and comparison panels shall expand; for summary generation, "
    "the report preview shall take focus; for job creation, the job submission form shall surface. "
    "Contextual action buttons (e.g., 'Generate Summary', 'Create Job', 'Compare Reports') shall "
    "appear dynamically based on the current content and workflow stage, enabling one-click transitions "
    "between workflows.")

add_req_table("FR8.8", "Progressive Disclosure",
    "The system shall present high-level summaries and KPI cards by default, with the ability to "
    "expand into detailed views on demand. Complex data (multi-period tables, sub-unit breakdowns, "
    "methodology notes) shall be collapsed unless the user explicitly requests expansion. Drill-down "
    "depth shall be consistent across all report types: overview > KPI detail > sub-unit detail > "
    "source data.")

add_req_table("FR8.9", "Dynamic Component Injection",
    "The system shall inject UI components dynamically into the chat workspace based on response type. "
    "When the query engine returns tabular data, an interactive data table shall render inline. When "
    "it returns KPI narratives, a formatted report preview panel shall appear. When a job is created "
    "from chat, a live job status tracker shall embed in the conversation. Injected components shall "
    "be interactive: users can sort tables, modify chart parameters, and trigger follow-up actions "
    "directly from within the chat context.")

add_req_table("FR8.10", "Multi-Panel Workspace",
    "The system shall support a multi-panel workspace layout with independently resizable panels: "
    "chat panel, document viewer, analysis/charting panel, and job board panel. Users shall be able "
    "to open, close, resize, and rearrange panels via drag-and-drop. Panel layout preferences shall "
    "be persisted per user across sessions. The workspace shall support split-view for side-by-side "
    "comparison of two reports, two time periods, or internal vs. competitor data.")

add_req_table("FR8.11", "UI State Management & Undo/Redo",
    "The system shall maintain full UI state across sessions, preserving: active documents, open "
    "panels, panel arrangement, chat history, selected filters, and dashboard widget configuration. "
    "When a user returns to the application, the workspace shall restore to its prior state. "
    "Dashboard and layout modifications shall support undo/redo (minimum 20 steps), allowing users "
    "to experiment with configurations without risk of losing a prior arrangement.")
...
add_table(
    ...
    [
        ["Period End Summary \u2014 Databricks path (first generation)", "< 12 seconds for 6 parallel KPI narratives + trend analysis"],
        ["Period End Summary \u2014 Excel fallback (first generation)", "< 15 seconds for 6 parallel KPI narratives + trend analysis"],
        ...
        ["Databricks view query", "< 5 seconds for any single precomputed view query"],
...
add_bullet("LLM provider: Azure OpenAI Foundry (GPT-4.1 or latest available model)")
...
add_bullet("Data source: Databricks/FinSight via SQL Connector or Unity Catalog REST API")
...
add_bullet("Graceful degradation: if Databricks is unavailable, fall back to Excel-based ingestion within 30 seconds")
...
doc.add_heading("4.1 Core Application Entities", level=2)
add_para(
    "The following entities represent FinIQ's application-level data model. These are supplemented "
    "by the Databricks/FinSight schema (Section 6) which provides the upstream financial data."
)
add_table(
    ["Entity", "Description", "Key Attributes"],
    [
        ["OrganizationalUnit", "Mars hierarchy node (maps to finiq_dim_entity)", "unit_id, name, parent_id, level, is_active"],
        ["FinancialPeriod", "Reporting period (maps to finiq_date)", "period_id, year, period_number, start_date, end_date, type"],
        ["KPIDefinition", "Configurable KPI spec (maps to finiq_account_formula + finiq_dim_account)", "kpi_id, name, formula, data_sources, prompt_template_id, thresholds"],
        ["FinancialFact", "Financial data point (maps to finiq_financial_cons + finiq_financial + finiq_financial_base)", "fact_id, unit_id, period_id, kpi_id, value, currency, source"],
...
doc.add_heading("4.2 Databricks/FinSight Entity Mapping", level=2)
add_para(
    "The following maps application entities to their Databricks source tables. Entities not "
    "present in Databricks are managed within FinIQ's application database."
)
add_table(
    ["Application Entity", "Databricks Table(s)", "Mapping Notes"],
    [
        ["OrganizationalUnit", "finiq_dim_entity", "Parent/Child hierarchy with Entity_Level; 150+ units"],
        ["FinancialPeriod", "finiq_date", "Date_ID (int), Year, Period, Quarter"],
        ["KPIDefinition", "finiq_account_formula + finiq_dim_account", "Account hierarchy + formula definitions"],
        ["FinancialFact", "finiq_financial_cons (primary) + finiq_financial + finiq_financial_base", "Three representations available"],
        ["(new) Product Taxonomy", "finiq_composite_item (12 cols) + finiq_item (15 cols) + finiq_item_composite_item (bridge)", "Full 27-attribute product master"],
        ["(new) Customer Dimension", "finiq_customer (11 cols) + finiq_customer_map (hierarchy)", "3-level customer hierarchy"],
        ["(new) Economic Cell", "finiq_economic_cell", "Economic grouping classification"],
        ["(new) Budget/Replan", "finiq_financial_replan (18 cols) + finiq_financial_replan_cons (6 cols)", "Actual vs. budget variance data"],
        ["(new) RLS Tracking", "finiq_rls_last_change", "Row-level security audit"],
        ["CompetitorCompany", "\u2014 (FinIQ app)", "Not in Databricks (CI data separate)"],
        ["CompetitorDocument", "\u2014 (FinIQ app)", "Not in Databricks (CI data separate)"],
        ["GeneratedReport", "\u2014 (Redis/Cosmos)", "FinIQ application entity"],
        ["Job", "\u2014 (Service Bus/SQL)", "FinIQ application entity"],
        ["DashboardConfig", "\u2014 (FinIQ app)", "FinIQ application entity"],
    ],
    col_widths=[1.5, 2.3, 2.7]
)

doc.add_heading("4.3 Data Classification", level=2)
add_table(
    ...
    [
        ...
        ["Databricks credentials", "Restricted", "Global Admin only", "Rotated per policy"],
    ],
...
add_bullet("LLM Service: Azure OpenAI Foundry (GPT-4.1 or latest) for narrative generation")
...
add_bullet("Cache Layer: Azure Redis Cache for generated reports, Databricks query results, and KPI summaries (JSON)")
...
add_table(
    ...
    [
        ...
        ["LLM Service", "Azure OpenAI Foundry", "Narrative generation, summarization, Q&A"],
        ...
        ["Dimension Sync Service", "Python + Azure Functions", "Periodic sync of Databricks dimension tables to local Azure SQL"],
        ["Data Freshness Monitor", "Python + Azure Functions", "Monitors Databricks data currency and triggers stale-data alerts"],
        ["Structured Data", "Azure SQL Database", "Application data, synchronized dimensions, KPIs"],
...
add_para("Flow 1: Period End Summary Generation (Databricks-Primary)", bold=True)
...
add_numbered("If Databricks unavailable: falls back to Excel workbook from Blob Storage (original pipeline)")
...
add_numbered("For factual/comparison: relevant data retrieved from Databricks (internal) and/or AI Search (external)")
...
# ══════════════════════════════════════════════════════════════════
#  6. DATABRICKS / FINSIGHT SCHEMA REFERENCE
# ══════════════════════════════════════════════════════════════════
doc.add_heading("6. Databricks / FinSight Schema Reference", level=1)

doc.add_heading("6.1 Schema Identity", level=2)
add_table(
    ["Property", "Value"],
    [
...
doc.add_heading("6.2 Object Inventory", level=2)
add_table(
...
doc.add_heading("6.3 Precomputed View Mapping to PES", level=2)

doc.add_heading("6.3.1 View-to-Excel Sheet Mapping", level=4)
add_table(
...
doc.add_heading("6.3.2 PES KPI-to-Account Code Mapping", level=4)
add_table(
...
doc.add_heading("6.4 View SQL Computation Patterns", level=2)
...
# ══════════════════════════════════════════════════════════════════
#  7. DUAL-MODE OPERATION: SIMULATED & REAL DATA
# ══════════════════════════════════════════════════════════════════
doc.add_heading("7. Dual-Mode Operation: Simulated and Real Data", level=1)

add_para(
    "Amira FinIQ shall support two operating modes \u2014 simulated data mode and real data mode \u2014 "
    "switchable via a single configuration toggle. This enables development and testing to proceed "
    "independently of Databricks access provisioning, and ensures the system can be validated "
    "end-to-end before connecting to production data."
)

doc.add_heading("7.1 Mode Definitions", level=2)
add_table(
    ["Mode", "Data Source", "Purpose", "When Used"],
    [
        ["Simulated", "Synthetic data matching Databricks schema exactly (column names, types, array structures, value ranges)", "Development, testing, demos, debugging, CI/CD pipelines", "Until real Databricks access is provisioned; for all automated testing thereafter"],
        ["Real", "Live Databricks/FinSight data layer (corporate_finance_analytics_dev.finsight_core_model_mvp3)", "Production operation with actual Mars financial data", "After Databricks credentials are provisioned and validated"],
    ],
    col_widths=[1.0, 2.5, 1.5, 1.5]
)

doc.add_heading("7.2 Configuration Toggle", level=2)
add_bullet("A single environment variable or admin setting (e.g., DATA_MODE=simulated|real) shall switch the entire system between modes")
add_bullet("The toggle shall affect all data ingestion, query, and reporting paths simultaneously")
add_bullet("Switching modes shall not require code changes or redeployment")
add_bullet("The current mode shall be clearly indicated in the UI (e.g., a banner: 'Running on simulated data')")

doc.add_heading("7.3 Simulated Data Requirements", level=2)
...
doc.add_heading("7.4 Simulated Data Delivery", level=2)
...
add_bullet("Integration tests shall run against both simulated and real Databricks environments with the same configuration toggle")
add_bullet("Simulated data shall be deterministic (seeded random) so test results are reproducible")

doc.add_heading("7.5 Mode Parity", level=2)
add_para(
    "All features shall behave identically in both modes. The only differences shall be:"
)
add_bullet("Data source connection (local DB vs. Databricks)")
add_bullet("UI mode indicator (banner showing simulated vs. real)")
add_bullet("Data freshness values (synthetic timestamps vs. real)")
add_para(
    "No feature shall be available in one mode but not the other. This ensures that validation "
    "in simulated mode provides confidence for real data operation.",
    italic=True
)
...
doc.add_heading("8.1 Infrastructure Requirements", level=2)
add_table(
    ...
    [
        ...
        ["LLM Service", "Azure OpenAI Foundry", "GPT-4.1 provisioned throughput (100K tokens/min)"],
        ["Embedding Service", "Azure OpenAI Foundry", "text-embedding-3-large"],
        ["Vector Search", "Azure AI Search", "Standard S1 (3 replicas)"],
        ["Databricks Connector", "Databricks SQL Warehouse", "Read-only access to finsight_core_model_mvp3"],
...
doc.add_heading("8.2 Deployment Model", level=2)
add_bullet("CI/CD via GitHub (Mars internal GitHub repository)")
...
doc.add_heading("8.3 Environments", level=2)
add_table(
    ["Environment", "Purpose", "Data Mode", "Access"],
    [
        ["Development", "Active development and unit testing", "Simulated", "Engineering team"],
        ["Staging", "Integration testing and UAT", "Simulated or Real (configurable)", "Engineering + QA + key stakeholders"],
        ["Production", "Live system", "Real", "All authorized users"],
    ],
...
#  9. PHASED ROLLOUT
# ══════════════════════════════════════════════════════════════════
doc.add_heading("9. Phased Rollout", level=1)

doc.add_heading("Phase 1: Foundation", level=2)
add_para("Goal: Consolidate current PES and CI tools into a unified platform with Databricks connectivity and core query interface.", bold=True)
add_bullet("Databricks/FinSight connectivity with fallback to Excel ingestion")
add_bullet("Period End Summary generation from precomputed views (all 6 KPIs, 3 output formats)")
add_bullet("Budget variance reporting (actual vs. replan)")
...
add_bullet("Admin dashboard (templates, KPIs, peer groups, Databricks connection management)")
add_bullet("Dynamic dashboard with configurable widget layout")
add_bullet("Dual-mode operation (simulated + real data)")
...
add_bullet("Amira Financial Forecasting API integration (three-way comparison: Actual vs. Replan vs. Forecast)")
...
add_table(
    ...
    [
        ["FR1.1 Databricks Ingestion", "System generates Period End Summary from Databricks views in < 12 seconds", "Automated comparison test: Databricks path vs. Excel path timing"],
        ...
        ["FR2.1 Period End Summary", "System generates 6-KPI executive summary for any unit/period in < 15 seconds (Excel) or < 12 seconds (Databricks)", "Automated load test with 10 concurrent requests"],
        ...
        ["FR6.1 Forecast Integration", "Three-way comparison (Actual vs. Replan vs. Forecast) renders correctly within 5 seconds", "Integration test with mock and live APIs"],
        ...
        ["FR8.1 Dynamic Dashboard", "Users can configure, save, and reload custom dashboard layouts", "UI test suite with 10 representative configurations"],
        ["FR8.3 Real-Time Updates", "Job status and report progress update within 500ms via SSE", "Automated SSE latency test"],
        ["FR8.7 Context-Aware Rendering", "UI layout adjusts within 300ms based on detected query intent", "Automated intent-to-layout mapping test"],
        ["FR8.10 Multi-Panel Workspace", "Users can open, resize, and persist 4-panel layouts across sessions", "UI regression test with panel state verification"],
        ["FR8.11 Undo/Redo", "Dashboard modifications support 20-step undo/redo without data loss", "Automated state consistency test"],
        ["Databricks Fallback", "System automatically falls back to Excel ingestion within 30 seconds of Databricks failure", "Chaos test: terminate Databricks connectivity mid-operation"],
        ["Dual-Mode Toggle", "System switches between simulated and real data modes without code changes or redeployment", "Configuration toggle test in staging environment"],
...
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run("Amira FinIQ SRS v3.0 (Final Unified) | Confidential | Prepared for Mars, Incorporated")
run.font.size = Pt(8)
run.font.color.rgb = RGBColor(0x99, 0x99, 0x99)
...
#  SAVE
# ══════════════════════════════════════════════════════════════════
output_path = r"D:\Amira FinIQ\FinIQ SRS v3.0 Final.docx"
