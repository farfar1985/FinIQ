...
- **Synthetic data COMPLETE (2026-03-26)** — Two versions available:
  - SQLite (working now): `finiq_synthetic.db` (21.4 MB, 165K rows, 17 tables + 3 views). Generated by `generate_synthetic_data_sqlite.py`. Share this with team immediately.
  - Databricks (blocked on permissions): `databricks_synthetic_data.py` needs write access. Cesar to resolve free tier permission issues (CREATE TABLE, SELECT, DROP denied on `workspace.default`).
...
- **Synthetic data script (Databricks)**: `databricks_synthetic_data.py` — 27-cell PySpark notebook, needs write permissions
- **Synthetic data script (SQLite)**: `generate_synthetic_data_sqlite.py` — standalone, works immediately
- **Synthetic database**: `finiq_synthetic.db` — 21.4 MB, 165K rows, ready to share
- **Permission issue**: Free tier blocks CREATE TABLE, SELECT, DROP on tables created by Spark runtime. Cesar needs to either upgrade tier or grant explicit permissions.
