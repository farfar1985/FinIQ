...
Version 2.1 — Merged: Claude base + best ChatGPT Dynamic UI ideas
"""
...
add_table(
    ...
    [
        ...
        ["FR8.7 Context-Aware Rendering", "UI layout adjusts within 300ms based on detected query intent (retrieval, analysis, summary, job)", "Automated intent-to-layout mapping test across 5 intent types"],
        ["FR8.10 Multi-Panel Workspace", "Users can open, resize, and persist 4-panel layouts across sessions", "UI regression test with panel state verification after session restore"],
        ["FR8.11 Undo/Redo", "Dashboard modifications support 20-step undo/redo without data loss", "Automated undo/redo state consistency test"],
...
p = doc.add_paragraph()
p.alignment = WD_ALIGN_PARAGRAPH.CENTER
run = p.add_run("Amira FinIQ SRS v2.1 | Confidential | Prepared for Mars, Incorporated")
run.font.size = Pt(8)
run.font.color.rgb = RGBColor(0x99, 0x99, 0x99)
...
#  SAVE
# ══════════════════════════════════════════════════════════════════
output_path = r"D:\Amira FinIQ\FinIQ SRS IEEE Format v2.1 Merged.docx"
